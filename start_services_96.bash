...
for corr in '03' 'h04' '05' '06' 'h07' '08' 'hh10' '11' 'h12' '14' '15' 'h16' '18' '19' '21' '22'; do
    echo "DOING ${corr} corr"; ssh h${corr}.pro.pvt "sudo loginctl enable-linger ubuntu; source ~/.bashrc; systemctl --user daemon-reload; systemctl --user stop corr.service; systemctl --user start corr.service"
    echo "DOING ${corr} mon"; ssh h${corr}.pro.pvt "sudo loginctl enable-linger ubuntu; source ~/.bashrc; systemctl --user restart mon.service"
    echo "DOING ${corr} voltage"; ssh h${corr}.pro.pvt "sudo loginctl enable-linger ubuntu; source ~/.bashrc; systemctl --user restart voltage.service"
    echo "DOING ${corr}"; ssh h${corr}.pro.pvt "sudo loginctl enable-linger ubuntu; source ~/.bashrc; systemctl --user restart cleanup.service"
    i=$((i + 1))    
done
